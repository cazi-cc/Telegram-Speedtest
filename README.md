...
- 使用具体 Telegram 频道/群组视频消息链接测速。
- 分别测试单连接敏感性和多连接总吞吐。
- 默认每轮只限时下载，不完整保存大视频。
- 默认推荐低资源方案：20 秒、多连接 4 线程、每轮 128MiB 磁盘上限。
- 退出、Ctrl+C、SSH 断开时自动清理临时视频、残片和日志。
...
- 500MB 以上的视频，最好 1GB 左右。
- 普通 MP4 或 Telegram 文件视频。
- 比较不同 VPS 时始终使用同一条消息链接。

脚本默认是限时下载测速，不会把大视频完整下载完。视频太小会提前完成，影响对比。
...
- 单连接慢、多连接快：节点总带宽可能不差，但 Telegram 视频首开和拖动缓冲可能差。
...
保留登录数据是为了避免每次重新扫码。需要彻底清理时，在菜单中选择：
