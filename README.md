...
- 登录前检查 NTP 时间同步；10 秒内没有收到二维码/扫码提示或手机号输入提示时，会提示用户确认是否修复时间同步问题，不会擅自修改系统。
...
- 如果 `tdl` 是本次脚本临时安装的，退出时默认保留，避免下次重复下载；可在清理菜单中切换为退出删除。
...
- 原始测速日志。
- 临时目录。
- 本次由脚本临时安装的 `tdl` 主程序默认保留；除非你在菜单中切换为退出删除。
